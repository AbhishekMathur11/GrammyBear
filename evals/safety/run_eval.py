...
def main() -> None:
    ...
    metrics_public = {k: v for k, v in metrics.items() if k != "unsafe_allow_examples"}
    metrics_public["unsafe_allow_ids"] = [r["id"] for r in metrics.get("unsafe_allow_examples", [])]
    (out_dir / "metrics.json").write_text(json.dumps(metrics_public, indent=2) + "\n")
